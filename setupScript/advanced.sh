...
function non_mongos_part1(){
    ...
    *) echo "Sorry, that is not a valid instance.";;
    ...
    chmod 600 /Users/mdas/Downloads/mickey-das.pem
    ssh -i /Users/mdas/Downloads/mickey-das.pem ubuntu@$1 << REMOTE_COMMANDS
...
REMOTE_COMMANDS
  ...
  echo -e "
    Please open a new terminal window and enter the following commands to setup the ports!
    chmod 600 Downloads/mickey-das.pem
    ssh -i Downloads/mickey-das.pem ubuntu@$1
...
# There is no such thing as mongos_part 1, we just do mongo setup
function mongos_part1(){
    chmod 600 Downloads/mickey-das.pem
    ssh -i Downloads/mickey-das.pem ubuntu@$1 << REMOTE_COMMANDS
...
REMOTE_COMMANDS
  ...
  echo -e "
    Please open a new terminal window and enter the following commands to setup the ports!
    chmod 600 /Users/mdas/Downloads/mickey-das.pem
    ssh -i /Users/mdas/Downloads/mickey-das.pem ubuntu@$1
    nohup mongos --configdb config_repl/$1:28041,$1:28042,$1:28043 --bind_ip localhost,$2 &
  "
}

function non_mongos_part2(){
    dir1="shard/"$3"rep1"
    dir2="shard/"$3"rep2"
    dir3="shard/"$3"rep3"
    name=""$3"_repl"
...
function mongos_part2(){
    chmod 600 Downloads/mickey-das.pem
    ssh -i Downloads/mickey-das.pem ubuntu@$6 << REMOTE_COMMANDS
...
while [ $idx -lt 5 ]
do
    echo "$[idx*3]: ${inst_arr[idx*3]}"
    echo "$[idx*3+1]: ${inst_arr[idx*3+1]}"
    echo -e "$[idx*3+2]: ${inst_arr[idx*3+2]}\n"
    let idx+=1
done
...
#while [ $idx -lt 1 ]
#do
#  echo -e "\n Executing Setup Part 1 for "${inst_arr[idx*3]}" instance. Shall we proceed?(y/n)\n"
#
#  read -p "" -n1 -s commandKey
#  if [ ! -z "$commandKey" ] && [ $commandKey = "y" ]; then
#      echo -e "\nOkay! Let's go!"
#      non_mongos_part1 "${inst_arr[idx*3+1]}" "${inst_arr[idx*3+2]}" "${inst_arr[idx*3]}"
#      echo "Please Complete nohup before proceeding to the next instance!"
#  elif [ ! -z "$commandKey" ] && [ $commandKey = "n" ]; then
#      echo -e "\n\nExiting..."
#      exit
#  fi
...
#while [ $idx -lt 1 ]
#do
#  echo -e "\n Executing Setup Part 2 for "${inst_arr[idx*3]}" instance. Shall we proceed?(y/n)\n"
#
#  read -p "" -n1 -s commandKey
#  if [ ! -z "$commandKey" ] && [ $commandKey = "y" ]; then
#      echo -e "\nOkay! Let's go!"
#      non_mongos_part2 "${inst_arr[idx*3+1]}" "${inst_arr[idx*3+2]}" "${inst_arr[idx*3]}"
#      echo "Please Complete nohup before proceeding to the next instance!"
#  elif [ ! -z "$commandKey" ] && [ $commandKey = "n" ]; then
#      echo -e "\n\nExiting..."
#      exit
